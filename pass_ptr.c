...
uint32_t solution[105][2]; //store solution
...
#define HELLO_WORLDD  2
#define LLS_START  3

uint32_t* debug;
#define DEBUG_AREA_RANGE  128  // need to be changed , depending your input data.
...
void hello_worldd(uint ts, uint object, uint vid ) {

    //state temp = *((state*) vid);
    set_log((*((state*) vid)).x);
    set_log((*((state*) vid)).y);
    (*((state*) vid)).x =20;
    (*((state*) vid)).y =21;
    ...
    //enq_task_arg1(HELLO_WORLD1, ts +1 , (uint32_t) &dist[0], vid );
}
...
void hello_world(uint ts, uint object, uint vid ) {
    ...
    enq_task_arg1(HELLO_WORLDD, ts +1 , (uint32_t) &dist[0], vid );
}
...
void visit_node_task(uint ts, uint write_addr, uint vid) { 
      ...
      enq_task_arg1(HELLO_WORLD, 0, (uint32_t) &dist[0], (uint)&stt);
      return; 
...
int main() {
   ...
   while (1) {
      ...
      //deq_task_argp(&ttype, &ts, &object,&ptrr);
      switch(ttype){
...
//output of this code is  -> 2835 (correct)
*/
